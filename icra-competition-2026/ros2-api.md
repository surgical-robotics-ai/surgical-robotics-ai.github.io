...
The simulated dVRK (AMBF and IsaacSim) supports the following subset of topics in the `PSM1` and `PSM2` namespaces (SUBJECT TO CHANGE; see above link for documentation):
...
* `jaw/move_jp`:  Set gripper trajectory goal
* `jaw/servo_jp`:  Set gripper position goal for low-level controller

### Cameras

The physical (OAK-D-SR) and simulated (AMBF and IsaacSim) stereo camera streams are provided by ROS topics (separate topics for left and right cameras). Details TBD.
